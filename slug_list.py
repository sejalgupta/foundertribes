...
def get_all_slugs(url_list):
    '''
    Gets the slugs with investor names
    Parameters: 
        * url_list (list): all the urls with investor lists to scrape
    Output: 
        * slugs (list) : all the slugs
    '''
    slugs = set()
    for url in url_list:
        try:
            index = url.rfind('-')
            if index != -1 and index > 42:
                #get substring with slug
                slug = url[42:index]
                slugs.add(slug)
        except:
            continue
...
def get_investor_information(url_list):
    '''
    ...
    investor_links = set()
    slug_list = get_all_slugs(url_list)
    for i, url in enumerate(slug_list):
        print(i, url)
...
if __name__ == "__main__":
    ...
    get_investor_information(page_urls)